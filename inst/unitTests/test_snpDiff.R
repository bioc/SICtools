...
test_snpDiff <- function(){
	...
	## check
	snpTest <- snpDiff(bam1,bam2,refFsa,'chr04',962501,1026983,pValueCutOff=1,baseDistCutOff=0)
	checkEquals(nrow(snpTest),16)
	checkEquals(subset(snpTest,p.value < 1e-10)$pos,c(962801,1026683))
	
	## check NULL
	checkTrue(is.null(snpDiff(bam1,bam2,refFsa,'chr04',962501,1026983,pValueCutOff=0,baseDistCutOff=0)))
